...
def post_image_pyramid(img_feats_c, img_pyramid, h, w, proj_fc):
    img_shape_c = (h, w)
    img_feats_c = F.interpolate(img_feats_c, size=img_shape_c, mode="bilinear", align_corners=True)
    img_feats_c = img_feats_c.squeeze(0).view(-1, h * w).transpose(0, 1)
    img_feats_c = proj_fc(img_feats_c)
    img_feats_c = img_feats_c.unsqueeze(0) # torch.Size([1, 256, 24, 32])
    img_feats_c = img_feats_c.transpose(1, 2).contiguous().view(1, -1, h, w) # torch.Size([1, 256, 24, 32])
    all_img_feats_c = img_pyramid(img_feats_c) # [torch.Size([1, 256, 24, 32]), torch.Size([1, 256, 12, 16]), torch.Size([1, 256, 6, 8])]
    ...
    return F.normalize(img_feats_c, p=2, dim=1)

def save_depth_image(depth_mat: torch.Tensor, filename: str = 'depth.png'):
    """Save depth image as PNG."""
    depth_image = depth_mat.squeeze(0).cpu().numpy()  # Remove batch dimension and convert to numpy
    depth_image_normalized = (depth_image / depth_image.max() * 255).astype('uint8')  # Normalize to [0, 255]
...
def project_to_depth(
    ...
    """Project point cloud to depth image."""
    ...
    u = (x * focal_x / z) + center_x
    v = (y * focal_y / z) + center_y

    # Round pixel coordinates and ensure they are within image bounds
    ...
    depth_mat = torch.full((height, width), float('inf'), device=u.device)  # Shape (480, 640)
    idx = (v * width + u).view(-1)

    # Gather valid depth values
    valid_mask = mask.view(-1)
    depth_values = (z * scaling_factor)[valid_mask]

    # Use a temporary tensor to store the minimum depth
    depth_mat_flat = depth_mat.view(-1)
    for i in range(len(depth_values)):
        if valid_mask[i]:
            depth_mat_flat[idx[i]] = min(depth_mat_flat[idx[i]], depth_values[i])

    if depth_limit is not None:
        depth_mat[depth_mat > depth_limit * scaling_factor] = float('inf')

    # Replace infinite values with zeros (or some other logic as needed)
    depth_mat[depth_mat == float('inf')] = 0

    save_depth_image(depth_mat)

    if return_mask:
        return depth_mat, mask
    else:
        return depth_mat