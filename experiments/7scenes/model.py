...
from vision3d.ops.se3 import apply_transform, inverse_transform
...
from utils import get_2d3d_node_correspondences, patchify, post_image_pyramid, project_to_depth, save_depth_image
from BPnP.BPnP import BPnP_m3d
import numpy as np
from PIL import Image
from depth_densify import name2densefunc
from copy import deepcopy


class Diff2I2P(nn.Module):
    def __init__(self, cfg):
        ...
        self.refer_coordinate = cfg.train.refer_coordinate
        ...
        self.enable_ms_loss = cfg.loss.coarse_loss.enable_ms_loss
        if cfg.model.diff_pnp_solver:
            self.diff_pnp_solver_type = "bpnp"
            self.diff_pnp_solver = BPnP_m3d.apply
        else:
            self.diff_pnp_solver = None
        if cfg.model.deformable_correspondences:
            # tanh
            if cfg.train.refer_coordinate:
                self.coord_offset_net = nn.Sequential(
                    nn.Linear(cfg.model.image_backbone.output_dim + cfg.model.point_backbone.output_dim + 6, 128),
                    nn.ReLU(),
                    nn.Linear(128, 3),
                )
            else:
                self.coord_offset_net = nn.Sequential(
                                            nn.Linear(cfg.model.image_backbone.output_dim + cfg.model.point_backbone.output_dim, 128),
                                            nn.ReLU(),
                                            nn.Linear(128, 3),
                                        )
            self.deformable_corr_point_l2_loss_weight = cfg.model.deformable_correspondences.point_l2_norm_weight
    def to_harmonic(self, input):
        M = input.shape[0]
        input = torch.cat([input, torch.ones((M, 1), device=input.device)], dim=1)
        return input
    ...
    def proj_3to2(self, xyz, intrinsic, extrinsic):
        ...
        # extrinsic     4*4     the extrinsic matrix of the sample image
        # output:
        ...
        xyz = torch.linalg.inv(extrinsic) @ xyz.T  # Using PyTorch for matrix inversion and multiplication
        uvd = intrinsic @ xyz[0:3]
        uvd = uvd.T
        uv = uvd[:, 0:2] / (uvd[:, -1:] + 1e-5)
        d = uvd[:, -1]
        return uv, d

    def proj_pc2dpt(self, ply, extrinsic, intrinsic, h, w, max_depth=15.0):
        uv, dpt = self.proj_3to2(ply, intrinsic, extrinsic)


        mask_w = (uv[:, 0] < w) & (uv[:, 0] >= 0)
        mask_h = (uv[:, 1] < h) & (uv[:, 1] >= 0)
        mask_d = (dpt > 0.05) & (dpt < max_depth)
        mask = mask_h & mask_w & mask_d

        uv = uv[mask].to(torch.int64)
        # uv = torch.round(uv[mask])
        dpt = dpt[mask]

        if len(dpt) < 0.3 * len(ply):
            return None

        result = torch.zeros((h,w), device=ply.device)  # Ensure result is on the same device


        indices = torch.argsort(dpt, descending=True)
    ...
    def save_depth(self, tar_path, dpt, scale=1):
        dpt = dpt * scale
        tmp = dpt
    ...
    def forward(self, data_dict, epoch=None):
        ...
        if self.refer_coordinate:
            img_xyz_feats = deepcopy(img_points.transpose(0, 1).reshape(1, 3, img_h, img_w))
            image = torch.cat((image, img_xyz_feats), dim=1)
            pcd_feats = deepcopy(pcd_points)
        # 2.1 Image backbone
        img_feats_list = self.img_backbone(image)
        img_feats_x = img_feats_list[-1]  # (B, C8, H/8, W/8), aka, (1, 512, 60, 80) s4
        img_feats_f = img_feats_list[0]  # (B, C2, H, W), aka, (1, 128, 480, 640) latent_s1

        ## save feats for multi-scale loss
        if self.enable_ms_loss:
            output_dict["img_feats_latent_s4"] = post_image_pyramid(img_feats_x, self.img_pyramid, self.img_h_c, self.img_w_c, self.transformer.img_in_proj)

        # 2.2 Point backbone
        pcd_feats_list = self.pcd_backbone(pcd_feats, data_dict)
        pcd_feats_c = pcd_feats_list[-1]  # (Nc, 1024) s4
        pcd_feats_f = pcd_feats_list[0]  # (Nf, 128) latent_s1

        ## save feats for multi-scale loss
        if self.enable_ms_loss:
            output_dict["pcd_feats_latent_s4"] = F.normalize(self.transformer.pcd_in_proj(pcd_feats_c), p=2, dim=1)
        ...
        img_feats_c, pcd_feats_c = self.transformer(
            img_feats_c.unsqueeze(0), # (1, 768, 512)
            img_pixels_c.unsqueeze(0),
            pcd_feats_c.unsqueeze(0), # (1, 129, 1024)
            pcd_points_c.unsqueeze(0),
        )

        # 3.3 Post-transformer image feature pyramid
        img_feats_c = img_feats_c.transpose(1, 2).contiguous().view(1, -1, self.img_h_c, self.img_w_c) # torch.Size([1, 256, 24, 32])
        all_img_feats_c = self.img_pyramid(img_feats_c) # [torch.Size([1, 256, 24, 32]), torch.Size([1, 256, 12, 16]), torch.Size([1, 256, 6, 8])]
        ...
        if not self.training:
            ...
            img_corr_points = img_points_f[img_corr_indices] # cant be used for training
            img_corr_pixels = img_pixels_f[img_corr_indices]
            pcd_corr_points = pcd_points_f[pcd_corr_indices]
            pcd_corr_pixels = pcd_pixels_f[pcd_corr_indices] # cant be used for training
            ...
            if self.coord_offset_net and img_corr_feats.shape[0] > 0:
                point_x_width = img_points_f[:, 0].max() - img_points_f[:, 0].min()
                point_y_width = img_points_f[:, 1].max() - img_points_f[:, 1].min()
                point_z_width = img_points_f[:, 2].max() - img_points_f[:, 2].min()
                if self.refer_coordinate:
                    corr_feats_coord = torch.cat([img_corr_feats, pcd_corr_feats, pcd_corr_points, img_corr_points], dim=1)
                else:
                    corr_feats_coord = torch.cat([img_corr_feats, pcd_corr_feats], dim=1)
                coord_offsets_rel = self.coord_offset_net(corr_feats_coord)
                coord_offsets = coord_offsets_rel[:, 0].unsqueeze(-1) * point_x_width, coord_offsets_rel[:, 1].unsqueeze(-1) * point_y_width, coord_offsets_rel[:, 2].unsqueeze(-1) * point_z_width
                coord_offsets = torch.cat(coord_offsets, dim=-1)
                pcd_corr_points += coord_offsets
                output_dict["coord_offsets"] = coord_offsets
                output_dict["pcd_corr_points"] = pcd_corr_points
                pcd_points_f[pcd_corr_indices] = pcd_corr_points
                output_dict["pcd_points_f"] = pcd_points_f
            ...
            img_corr_pixels = img_pixels_f[img_corr_indices] # gt pixels correspondences
            pcd_corr_points = pcd_points_f[pcd_corr_indices] # gt points correspondences
            ...
            if self.coord_offset_net:
                if corr_scores.shape[0] > 0:
                    ...
                    coord_offsets = coord_offsets_rel[:, 0].unsqueeze(-1) * point_x_width, coord_offsets_rel[:, 1].unsqueeze(-1) * point_y_width, coord_offsets_rel[:, 2].unsqueeze(-1) * point_z_width
                    coord_offsets = torch.cat(coord_offsets, dim=-1)
                    pcd_corr_points += coord_offsets # estimated points correspondences
                    output_dict["coord_offsets"] = coord_offsets

                    # l2_norm loss
                    if self.deformable_corr_point_l2_loss_weight > 0.:
                        loss_coord = coord_offsets_rel.norm(dim=-1).mean()
                        output_dict["loss_coord"] = loss_coord

                    # gt coords residual l2 loss
                    inverted_transform = inverse_transform(transform)
                    aligned_img_corr_points = apply_transform(img_corr_points, inverted_transform)
                    offsets_residual = aligned_img_corr_points - pcd_corr_points
                    offsets_residual_rel = offsets_residual[:, 0].unsqueeze(-1) / point_x_width, offsets_residual[:, 1].unsqueeze(-1) / point_y_width, offsets_residual[:, 2].unsqueeze(-1) / point_z_width
                    offsets_residual_rel = torch.cat(offsets_residual_rel, dim=-1)
                    loss_gt_coord = offsets_residual_rel.norm(dim=-1).mean()
                    output_dict["loss_coord_residual"] = loss_gt_coord

                else:
                    print("No correspondences found")
                    # cancel grad for self.coord_offset_net
                    if self.refer_coordinate:
                        corr_feats = torch.zeros(1, 256 + 6).to(corr_scores.device)
                    else:
                        corr_feats = torch.zeros(1, 256).to(corr_scores.device)
                    coord_offsets = self.coord_offset_net(corr_feats)

                    loss_coord = coord_offsets.norm(dim=-1).mean() * 0.

                    output_dict["loss_coord"] = loss_coord
                    output_dict["loss_coord_residual"] = loss_coord

                output_dict["pcd_corr_points"] = pcd_corr_points
                pcd_points_f[pcd_corr_indices] = pcd_corr_points
                output_dict["pcd_points_f"] = pcd_points_f

            if corr_scores.shape[0] > 0 and self.diff_pnp_solver:
                num_corrs = corr_scores.shape[0]
                # if num_corrs >= 4 and num_corrs <= 8500:
                if num_corrs >= 4:
                    camera_intrinsics = data_dict["intrinsics"]
                    x3d = pcd_corr_points.unsqueeze(0)
                    x2d = img_corr_pixels.unsqueeze(0)
                    pose = self.diff_pnp_solver(x2d, x3d, camera_intrinsics)
                    axis_angle_rot_pred = pose[:, :3]
                    translation_pred = pose[:, 3:]
                    rot_mat_pred = T.axis_angle_to_matrix(axis_angle_rot_pred)
                    transform_pred = torch.eye(4).to(transform)
                    transform_pred[:3, :3] = rot_mat_pred
                    transform_pred[:3, 3] = translation_pred.squeeze(0)
                    residual_loss = apply_transform(pcd_corr_points, transform_pred) - img_corr_points
                    rot_mat_loss  = residual_loss.norm(dim=-1).mean()
                    output_dict["rot_mat_loss"] = rot_mat_loss

                    if epoch is not None and epoch > 30:
                        transformed_pcd = apply_transform(pcd_points, transform_pred)
                        dpt = self.proj_pc2dpt(transformed_pcd, extrinsic=torch.eye(4, device=transform_pred.device),
                                               intrinsic=intrinsics,
                                               h=480, w=640)
                        if dpt is not None:
                            dpt = name2densefunc['fast_differentiable'](dpt)
                            factor = 1 / (dpt.max() + 1e-8)
                            dpt = dpt * factor

                            output_dict["depth"] = dpt

                            with torch.no_grad():
                                gt_transformed_pcd = apply_transform(pcd_points, transform)
                                gt_dpt = self.proj_pc2dpt(gt_transformed_pcd,
                                                       extrinsic=torch.eye(4, device=transform_pred.device),
                                                       intrinsic=intrinsics,
                                                       h=480, w=640)
                                gt_dpt = name2densefunc['fast_differentiable'](gt_dpt)
                                output_dict['gt_depth'] = gt_dpt



                else:
                    print("Not enough or too much correspondences for PnP solver")
                    rot_mat_loss = torch.tensor(0.).to(corr_scores.device)
                    output_dict["rot_mat_loss"] = rot_mat_loss
