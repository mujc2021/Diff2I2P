    ...
    def backward(ctx, grad_output):
        ...
        for i in range(bs):
            ...
            for j in range(m):
                ...
                R = T.axis_angle_to_matrix(P_6d_flat[0:m-3].view(1,3))
    ...
    def forward(ctx, pts2d, pts3d, K, ini_pose=None):
        ...
        for i in range(bs):
            ...
            if ini_pose is None:
                ...
                # else:
                    # _, rvec0, T0, _ = cv.solvePnPRansac(objectPoints=pts3d_i_np, imagePoints=pts2d_i_np, cameraMatrix=K_np, distCoeffs=None, flags=cv.SOLVEPNP_EPNP, confidence=0.9999 ,reprojectionError=1)
            else:
                rvec0 = np.array(ini_pose[i, 0:3].cpu().view(3, 1))
                T0 = np.array(ini_pose[i, 3:6].cpu().view(3, 1))
...
def batch_project(P, pts3d, K, angle_axis=True):
    ...
    if angle_axis:
        R_out = T.axis_angle_to_matrix(P[:, 0:3].view(bs, 3))
